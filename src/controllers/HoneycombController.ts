...
export default class HoneycombController {
  ...
  private static saveDerivedColumnToSource(
  ...
  public static pull(uri: Uri) {
    ...
    if (id) {
      HoneycombController._hnyapi.get_one_derived_column(
        dataset,
        id,
        (dc: DerivedColumn | HoneycombError) => {
          if (isError(dc)) {
          ...
          Config.set("dataset_settings", dataset_settings).then(
            (success: any) => {}
          );
        }
      );
      ...
      window.showErrorMessage(
        "ID not saved locally. Please pull all from dataset to get DerivedColumn id."
      );
    }
  }
  ...
  public static pullAll(uri: Uri) {
    ...
    // }
    HoneycombController._hnyapi.get_all_columns(
      dataset,
      (dataset_columns: any) => {
        dataset_settings[dataset].columns = dataset_columns;
        HoneycombController._hnyapi.get_all_derived_columns(
          dataset,
          (columns: any) => {
            if (columns.error) {
  ...
  private static create_new_derived_column(dataset: string, dc: DerivedColumn) {
    let dataset_settings = Config.get("dataset_settings");
    HoneycombController._hnyapi.create_new_derived_column(
      dataset,
      dc,
      (col: DerivedColumn | HoneycombError) => {
        if (isError(col)) {
          ...
          window.showInformationMessage(
            `Created DerivedColumn ${col.alias} with id ${col.id}`
          );
          Config.set("dataset_settings", dataset_settings).then(
            (success: any) => {}
          );
  ...
  private static update_derived_column(dataset: string, dc: DerivedColumn) {
    HoneycombController._hnyapi.update_derived_column(
      dataset,
      dc.id as string,
      dc,
      (col: DerivedColumn | HoneycombError) => {
        if (isError(col)) {
          if (col.error === 'derived column not found') {
            window.showWarningMessage('DerivedColumn not found, creating new DerivedColumn');
            delete dc["id"];
    ...
    workspace.openTextDocument(uri.fsPath).then((document: TextDocument) => {
      ...
      if (dataset_settings[dataset].derived_columns[alias]) {
        ...
        let dc: DerivedColumn = {
          id: id,
          description:
            dataset_settings[dataset].derived_columns[alias].description,
          alias: alias,
  ...
  public static query(uri: Uri) {
    ...
    HoneycombController._hnyapi.create_query(
      ...
      (query: any) => {
        if (query.id) {
          HoneycombController._hnyapi.create_query_result(
            dataset,
            query.id,
            (query_result: any) => {
              if (query_result.id) {
                HoneycombController._hnyapi.get_query_result(
                  dataset,
                  query_result.id,
                  (query_data_result: any) => {
